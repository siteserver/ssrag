var utils = {
  ...
  isAppSite: function (site) {
    return site.siteType === "Chat" || site.siteType === "Chatflow" || site.siteType === "Agent";
  },

  getSiteTypeName: function (siteType) {
    return siteType === "Chat"
      ? "对话应用"
      : siteType === "Chatflow"
      ? "工作流应用"
      : siteType === "Agent"
  ...
  getDatasetUrl: function (name, query) {
    return utils.getPageUrl("dataset", name, query);
  },
  ...
  getFriendlyDate: function (date) {
    ...
    return (
      date.getFullYear() + "-" + utils.pad(date.getMonth() + 1) + "-" + utils.pad(date.getDate())
    );
  },
  ...
  openDocs: function (url) {
    ...
    window.open("https://ssrag.com/docs/handbook/" + url, "_docs");
  },

  keyPress: function (submitFn, cancelFn) {
    $(document).keydown(function (e) {
      if (
        (e.ctrlKey && e.key !== "Control" && (e.keyCode || e.which) == 83) ||
        (e.ctrlKey && e.which == 13) ||
        e.which == 10 ||
        (e.shiftKey && e.which == 13) ||
        e.which == 10
      ) {
        e.preventDefault();
        e.stopPropagation();
  ...
  getValue: function (styles, content, attributeName) {
    ...
    for (var i = 0; i < styles.length; i++) {
      var style = styles[i];
      if (style.attributeName !== attributeName || !style.items || style.items.length === 0)
        continue;
...
var SESSION_ID_NAME = "ssrag_session_id";
var ACCESS_TOKEN_NAME = "ssrag_access_token";
