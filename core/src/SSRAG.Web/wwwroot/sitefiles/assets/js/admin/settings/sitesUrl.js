...
var data = utils.init({
  ...
  editLoading: false,
  editRules: {
    separatedWebUrl: [{ required: true, message: "独立部署知识库访问地址", trigger: "blur" }],
    assetsDir: [{ required: true, message: "上传文件存储文件夹", trigger: "blur" }],
    separatedAssetsUrl: [{ required: true, message: "独立部署上传文件访问地址", trigger: "blur" }],
    separatedApiUrl: [{ required: true, message: "独立部署API访问地址", trigger: "blur" }],
  },
});
