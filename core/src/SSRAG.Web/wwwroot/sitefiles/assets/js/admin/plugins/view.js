...
var methods = {
  apiGet: function () {
    ...
    $api
      ...
      .then(function (response) {
        ...
        cloud
          .getExtension($this.cmsVersion, publisher, name)
          .then(function (response) {
            var res = response.data;
            ...
            if ($this.plugin) {
              $this.isShouldUpdate =
                cloud.compareVersion($this.plugin.version, $this.release.version) == -1;
            }
  ...
  getPluginUrl: function () {
    if (this.extension) {
      return (
        cloud.host +
        "/market/extension/?userName=" +
        encodeURIComponent(this.extension.userName) +
        "&name=" +
        encodeURIComponent(this.extension.name)
      );
    }
    return "javascript:;";
  },
  ...
  btnBuyClick: function () {
    ...
    cloud.checkAuth(function () {
      utils.openLayer({
        ...
        url:
          cloud.host +
          "/layer/pay.html?resourceType=Extension&userName=" +
