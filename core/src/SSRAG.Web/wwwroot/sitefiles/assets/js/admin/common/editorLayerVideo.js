...
var methods = {
  ...
  cloudUploadBefore: function (file) {
    this.uploadErrorMessage = "";
    var re =
      /(\.3gp|\.asf|\.avi|\.dat|\.dv|\.flv|\.f4v|\.gif|\.m2t|\.m4v|\.mj2|\.mjpeg|\.mkv|\.mov|\.mp4|\.mpe|\.mpg|\.mpeg|\.mts|\.ogg|\.qt|\.rm|\.rmvb|\.swf|\.ts|\.vob|\.wmv|\.webm)$/i;
  ...
  uploadRequest: function (data) {
    this.uploadUrl =
      $url +
      "/actions/uploadVideo?siteId=" +
      this.form.siteId +
      "&isChangeFileName=" +
      this.form.isChangeFileName;
    ...
    var config = {
      onUploadProgress: function (progressEvent) {
        $this.uploadProgressPercent = Number(
          ((progressEvent.loaded / progressEvent.total) * 30).toFixed(2)
        );
  ...
  apiGet: function () {
    ...
    $api
      ...
      .then(function (response) {
        ...
        $this.reExtensions = new RegExp(
          "(" + res.videoUploadExtensions.replace(/,/g, "|").replace(/\./g, "\\.") + ")$",
          "i"
        );
  ...
  btnSubmitClick: function () {
    ...
    var imageUrl =
      this.form.isPoster && this.form.imageVirtualUrl
        ? ' imageUrl="' + this.form.imageVirtualUrl + '"'
        : "";
    ...
    parent.$vue.insertEditor(
      $this.attributeName,
      '<img src="/sitefiles/assets/images/video-clip.png" style="width: 333px; height: 333px"' +
        imageUrl +
        isAutoPlay +
        width +
        height +
        ' playUrl="' +
        this.form.videoUrl +
        '" class="siteserver-stl-player" /><br/>'
    );
    utils.closeLayer();
  },
  ...
  cloudUploadRequest: function (data) {
    ...
    var config = {
      onUploadProgress: function (progressEvent) {
        $this.cloudUploadProgressPercent = Number(
          ((progressEvent.loaded / progressEvent.total) * 30).toFixed(2)
        );
        if (progressEvent.loaded === progressEvent.total) {
