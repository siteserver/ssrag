...
var methods = {
  ...
  apiDatabaseConnect: function () {
    ...
    utils.loading(this, true);
    $api
      .post($url + "/actions/databaseConnect", this.databaseForm)
      .then(function (response) {
        var res = response.data;
  ...
  apiRedisConnect: function () {
    ...
    utils.loading(this, true);
    $api
      .post($url + "/actions/redisConnect", this.redisForm)
      .then(function (response) {
        var res = response.data;
  ...
  apiPrepare: function () {
    ...
    $api
      .post(
        $url + "/actions/prepare",
        _.assign({}, this.databaseForm, this.redisForm, this.adminForm)
      )
      .then(function (response) {
        var res = response.data;
  ...
  apiInstall: function (securityKey) {
    ...
    $api
      .post(
        $url + "/actions/install",
        _.assign({ securityKey: securityKey }, $this.databaseForm, $this.redisForm, $this.adminForm)
      )
      .then(function (response) {
        var res = response.data;
        $this.pageIndex++;
  ...
  checkPasswordLevel: function (strPassword) {
    ...
    for (var i = 0; i < strPassword.length; i++) {
      if (
        (strPassword.charAt(i) >= "0" && strPassword.charAt(i) <= "9") ||
  ...
  btnDatabaseConnectClick: function () {
    ...
    if (this.containerized) {
      ...
      if (
        (this.databaseNames && this.databaseForm.databaseName) ||
        this.databaseForm.databaseType === $sqlite
      ) {
        this.pageIndex++;
        return;
      }
