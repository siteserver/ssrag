...
var methods = {
  ...
  apiAppend: function () {
    ...
    $api
      .post($url + "/actions/append", {
        ...
        isKnowledge: this.appendForm.isKnowledge,
  ...
  apiUpdate: function () {
    ...
    utils.loading(this, true);
    $api
      .post($urlUpdate, this.form)
      .then(function (response) {
        var res = response.data;

        $this.editPanel = false;
        $this.apiList(res.channelIds);
        utils.success("栏目编辑成功!");

        if (res.datasetIndex) {
          $aiApi.tasksIndex($this.siteId, $this.form.id, 0);
        }
        if (res.datasetRemove) {
          $aiApi.tasksRemove($this.siteId, $this.form.id, 0);
        }
      })
      .catch(function (error) {
        utils.loading($this, false);
  ...
  apiDelete: function () {
    ...
    $api
      ...
      .then(function (response) {
        ...
        $aiApi.tasksRemove($this.siteId, $this.deleteForm.channelId, 0);
      })
      .catch(function (error) {
        utils.loading($this, false);
        utils.error(error);
  ...
  filterNode: function (value, data) {
    ...
    if (value.channelName && value.indexName && value.groupName) {
      return (
        (data.label.indexOf(value.channelName) !== -1 || data.value + "" === value.channelName) &&
        data.channel.indexName === value.indexName &&
        data.groupNames.indexOf(value.groupName) !== -1
      );
