...
var methods = {
      ...
      .then(function (response) {
        ...
        if (res.value) {
          ...
          if (res.isCloudAdmin) {
            ...
            $this.adminTitle = res.adminTitle || "SSRAG 管理后台";
            ...
            $this.adminLogoLinkUrl = "https://ssrag.com";
            adminFaviconUrl = utils.getAssetsUrl("images/favicon.png");
            $this.adminTitle = "SSRAG 管理后台";
  ...
  getUpdates: function () {
    ...
    var pluginIds = this.plugins.map(function (x) {
      return x.pluginId;
    });
  ...
  btnSearchClick: function () {
    ...
    utils.addTab(
      "内容搜索",
      utils.getCmsUrl("contentsSearch", {
        siteId: this.siteId,
  ...
  btnUserMenuClick: function (command) {
    if (command === "view") {
      utils.openLayer({
        title: "查看资料",
        url: utils.getCommonUrl("adminLayerView", { uuid: this.local.uuid }),
        full: true,
      });
