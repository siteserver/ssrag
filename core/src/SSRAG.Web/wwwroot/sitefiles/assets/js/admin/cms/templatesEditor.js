...
var data = utils.init({
  ...
  createdFileFullNameTips: "以“~/”开头代表系统根目录，以“@/”开头代表知识库根目录",
  templateName: null,
  relatedFile: null,
  channels: null,
  contents: null,
  content: null,
  contentEditor: null,
  createdFileExtNames: [".html", ".htm", ".shtml", ".xml", ".txt", ".json", ".js"],
...
var methods = {
    ...
    $api
      ...
      .then(function (response) {
        ...
        value = html.replace(
          ...
          "<head><base href='" +
            baseUrl +
            "' /><script>if ( window.innerWidth === 0 ) { window.innerWidth = parent.innerWidth; window.innerHeight = parent.innerHeight; }</script>"
        );
  ...
  btnPreviewClick: function () {
    if (this.settings.templateId == 0) {
      ...
      (this.dataSource.channelId === 0 ||
        !this.dataSource.contentId ||
        this.dataSource.contentId === 0) &&
      this.templateType === "ContentTemplate"
    ) {
      this.panelDataSource = true;
      this.next = "preview";
    } else {
  ...
  btnDataSourceSubmitClick: function () {
    ...
    this.$refs.dataSource.validate(function (valid) {
      if (valid) {
        ...
        $this.dataSource.channelId =
          $this.dataSource.channelIds[$this.dataSource.channelIds.length - 1];
