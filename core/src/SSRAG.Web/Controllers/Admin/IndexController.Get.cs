...
using SSRAG.Utils;
using SSRAG.Web.Controllers.Admin.Settings.Sites;
using SSRAG.Datory;
using SSRAG.Enums;
...
namespace SSRAG.Web.Controllers.Admin
{
    public partial class IndexController
    {
        ...
        public async Task<ActionResult<GetResult>> Get([FromQuery] GetRequest request)
        {
            ...
            {
                var envSecurityKey = SettingsManager.GetEnvironmentVariable(SettingsManager.SECURITY_KEY);
                var envDbHost = SettingsManager.GetEnvironmentVariable(SettingsManager.DB_HOST);
                var envDbUser = SettingsManager.GetEnvironmentVariable(SettingsManager.DB_USER);
                var envDbPassword = SettingsManager.GetEnvironmentVariable(SettingsManager.DB_PASSWORD);
                var envDbDatabase = SettingsManager.GetEnvironmentVariable(SettingsManager.DB_DATABASE);

                if (string.IsNullOrEmpty(envSecurityKey) || string.IsNullOrEmpty(envDbHost) || string.IsNullOrEmpty(envDbUser) || string.IsNullOrEmpty(envDbPassword) || string.IsNullOrEmpty(envDbDatabase))
                {
                    return this.Error("系统启动失败，请检查 SSRAG 容器运行环境变量设置");
                }
            }
            ...
            var siteType = SiteType.Web;
            var previewUrl = string.Empty;
            if (site != null)
            {
                siteType = site.SiteType;
                var siteTypeStr = site.SiteType.GetValue();
                if (await _authManager.HasSitePermissionsAsync(site.Id))
                {
                    ...
                    foreach (var enabledPlugin in enabledPlugins)
                    {
                        var pluginMenus = enabledPlugin.GetMenus()
                            .Where(x => ListUtils.ContainsIgnoreCase(x.Type, siteTypeStr)).ToList();
                    ...
                    var siteMenus = allMenus
                        .Where(menu => ListUtils.ContainsIgnoreCase(menu.Type, siteTypeStr))
                    ...
                    if (siteIdListWithPermissions.Count > 1)
                    {
                        ...
                        foreach (var siteId in siteIdList)
                        {
                            ...
                            var theSiteType = theSite.SiteType;
                            allSiteMenus.Add(new Menu
                            {
                                Id = $"site_switch_{theSite.Id}",
                                IconClass = _siteRepository.GetSiteTypeIconClass(theSiteType),
                ...
                siteUrl = _pathManager.GetSiteUrl(site, false);
