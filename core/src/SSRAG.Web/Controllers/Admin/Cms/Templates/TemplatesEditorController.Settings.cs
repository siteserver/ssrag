    ...
    public partial class TemplatesEditorController
    {
        ...
        public async Task<ActionResult<SettingsResult>> Settings([FromBody] SettingsRequest request)
        {
            ...
            if (request.Settings.TemplateId > 0)
            {
                ...
                await _pathManager.WriteContentToTemplateFileAsync(site, template, request.Content, _authManager.AdminName);

                await _templateRepository.UpdateAsync(template);
                if (previousTemplate != null)
                {
                    FileUtils.DeleteFileIfExists(_pathManager.GetTemplateFilePath(site, previousTemplate));
                }
