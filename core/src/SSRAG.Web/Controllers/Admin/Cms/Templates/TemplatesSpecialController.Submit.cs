...
namespace SSRAG.Web.Controllers.Admin.Cms.Templates
{
    public partial class TemplatesSpecialController
    {
        ...
        public async Task<ActionResult<SubmitResult>> Submit([FromBody] SubmitRequest request)
        {
            ...
            var siteDirectoryPath = _pathManager.GetSitePath(site);
            ...
            if (specialId > 0 && request.IsEditOnly)
            {
                ...
                if (specialInfo.Url != url)
                {
                    ...
                    oldDirectoryPath = _pathManager.GetSpecialDirectoryPath(site, specialInfo.Url);
                    newDirectoryPath = _pathManager.GetSpecialDirectoryPath(site, url);

                    if (!DirectoryUtils.IsInDirectory(siteDirectoryPath, oldDirectoryPath))
                    {
                        return this.Error("专题访问地址必须在知识库访问地址内部！");
                    }
            ...
            else if (specialId > 0 && request.IsUploadOnly)
            {
                ...
                var uploadDirectoryPath = _pathManager.GetTemporaryFilesPath(request.Uuid);
            ...
            else if (specialId == 0)
            {
                ...
                var directoryPath = _pathManager.GetSpecialDirectoryPath(site, url);
                if (!DirectoryUtils.IsInDirectory(siteDirectoryPath, directoryPath))
                {
                    return this.Error("专题访问地址必须在知识库访问地址内部！");
                }
