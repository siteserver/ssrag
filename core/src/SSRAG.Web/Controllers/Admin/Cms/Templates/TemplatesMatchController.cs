        ...
        private async Task CreateChannelTemplateAsync(Site site, CreateRequest request)
        {
            ...
            foreach (var channelId in request.ChannelIds)
            {
                ...
                if (channelTemplateId != -1)
                {
                    ...
                    var filePath = _pathManager.GetTemplateFilePath(site, template);
                    if (!FileUtils.IsFileExists(filePath))
                    {
                        await _pathManager.WriteContentToTemplateFileAsync(site, template, Constants.Html5Empty, _authManager.AdminName);
                    }
                }
            }
        }
