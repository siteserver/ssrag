...
namespace SSRAG.Core.StlParser.StlElement
{
    ...
    public static class StlForm
    {
        ...
        private static async Task<object> ParseAsync(IParseManager parseManager, ParseContext context, Site site, Form form, string apiUrl, string type, string height, NameValueCollection attributes)
        {
            ...
            var formTemplate = parseManager.FormManager.GetFormTemplate(site, type);
            if (formTemplate == null)
            {
                formTemplate = parseManager.FormManager.GetFormTemplate(site, "submit");
            }
            ...
            {
                pageUrl = parseManager.PathManager.GetSiteUrl(site, pathWithQuery, false);
            }
