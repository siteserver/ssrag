...
namespace SSRAG.Core.StlParser.StlElement
{
    ...
    public static class StlSite
    {
        ...
        private static async Task<string> ParseAsync(IParseManager parseManager, Site site, string type, string format, string no, string separator, int startIndex, int length, int wordNum, string ellipsis, string replace, string to, bool isClearTags, bool isClearBlank, string isReturnToBrStr, bool isLower, bool isUpper, NameValueCollection attributes)
        {
            ...
            else if (StringUtils.EqualsIgnoreCase(type, TypeImageUrl))
            {
                ...
                if (no == "all")
                {
                    ...
                    sbParsedContent.Append(contextInfo.IsStlEntity
                        ? parseManager.PathManager.ParseSiteUrl(site, site.ImageUrl, pageInfo.IsLocal)
                        : inputParser.GetImageOrFlashHtml(site, site.ImageUrl, attributes, false));
                    ...
                    for (var i = 1; i <= count; i++)
                    {
                        ...
                        sbParsedContent.Append(contextInfo.IsStlEntity
                            ? parseManager.PathManager.ParseSiteUrl(pageInfo.Site, extend, pageInfo.IsLocal)
                            : inputParser.GetImageOrFlashHtml(pageInfo.Site, extend, attributes, false));
                    }
            ...
            {
                parsedContent = parseManager.PathManager.GetWebUrl(site);
            }
                ...
                if (!string.IsNullOrEmpty(parsedContent))
                {
                    ...
                    if (styleInfo.Id > 0)
                    {
                        ...
                        else
                        {
                            ...
                            parsedContent = inputParser.GetContentByTableStyle(parsedContent, separator, pageInfo.Site, styleInfo, format, attributes, contextInfo.InnerHtml, contextInfo.IsStlEntity);
