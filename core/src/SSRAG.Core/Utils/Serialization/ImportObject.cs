...
namespace SSRAG.Core.Utils.Serialization
{
    public class ImportObject
    {
        ...
        public ImportObject(IPathManager pathManager, IDatabaseManager databaseManager, Caching caching, Site site, string adminName)
        {
        ...
        public async Task ImportFilesAsync(string siteTemplatePath, bool isOverride, string uuid)
        {
            var sitePath = _pathManager.GetSitePath(_site);
            ...
            foreach (var filePath in filePaths)
            {
                ...
                await _caching.SetProcessAsync(uuid, $"导入知识库文件: {filePath}");
                FileUtils.MoveFile(filePath, destFilePath, isOverride);
            }

            if (_site.Root)
            {
                siteDirList = await _databaseManager.SiteRepository.GetSiteDirsAsync();
            }
            ...
            foreach (var subDirectoryPath in directoryPaths)
            {
                ...
                await _caching.SetProcessAsync(uuid, $"导入知识库文件夹: {subDirectoryPath}");
            ...
            var siteTemplateMetadataPath = PathUtils.Combine(sitePath, DirectoryUtils.SiteFiles.Templates.SiteTemplateMetadata);
            DirectoryUtils.DeleteDirectoryIfExists(siteTemplateMetadataPath);
        }

        public async Task ImportSiteContentAsync(string siteContentDirectoryPath, string filePath, bool isImportContents, string uuid)
        {
            var siteIe = new SiteIe(_pathManager, _databaseManager, _caching, _site, siteContentDirectoryPath);
            await siteIe.ImportChannelsAndContentsAsync(filePath, isImportContents, false, 0, _adminName, uuid);
        }

        public async Task ImportTemplatesAsync(string filePath, bool overwrite, string adminName, string uuid)
        {
            var templateIe = new TemplateIe(_pathManager, _databaseManager, _caching, _site, filePath);
            await templateIe.ImportTemplatesAsync(overwrite, adminName, uuid);
        }

        public async Task ImportFormsAsync(string formDirectoryPath, string uuid)
        {
            var formIe = new FormIe(_pathManager, _databaseManager, _caching, _site.Id, formDirectoryPath);
            await formIe.ImportFormsAsync(uuid);
        }
        ...
        public async Task ImportTableStylesAsync(string tableDirectoryPath, string uuid)
        {
            if (DirectoryUtils.IsDirectoryExists(tableDirectoryPath))
            {
                ...
                await tableStyleIe.ImportTableStylesAsync(_site, uuid);
            }
        }
        ...
        public async Task ImportConfigurationAsync(string configurationFilePath, string uuid)
        {
            var configIe = new ConfigurationIe(_databaseManager, _caching, _site, configurationFilePath);
            await configIe.ImportAsync(uuid);
        }

        public async Task ImportChannelsAndContentsByZipFileAsync(int parentId, string zipFilePath, bool isOverride, bool isContents, string uuid)
        {
            ...
            await ImportChannelsAndContentsFromZipAsync(parentId, siteContentDirectoryPath, isOverride, isContents, uuid);
        ...
        private async Task ImportChannelsAndContentsFromZipAsync(int parentId, string siteContentDirectoryPath, bool isOverride, bool isContents, string uuid)
        {
            ...
            foreach (var filePath in filePathList)
            {
                ...
                var insertChannelId = await siteIe.ImportChannelsAndContentsAsync(filePath, isContents, isOverride, (int)levelHashtable[level], _adminName, uuid);
                levelHashtable[level + 1] = insertChannelId;
            }
        }

        public async Task ImportChannelsAndContentsAsync(int parentId, string siteContentDirectoryPath, bool isOverride, string uuid)
        {
            var filePathList = GetSiteContentFilePathList(siteContentDirectoryPath);
            ...
            foreach (var filePath in filePathList)
            {
                ...
                {
                    parentId = await siteIe.ImportChannelsAndContentsAsync(filePath, true, isOverride, parentId, _adminName, uuid);
                    parentOrderString = orderString;
                }
                else
                {
                    await siteIe.ImportChannelsAndContentsAsync(filePath, true, isOverride, parentId, _adminName, uuid);
                }
            }
        }

        public async Task<List<int>> ImportContentsByZipFileAsync(Channel channel, string zipFilePath, bool isOverride, bool isChecked, int checkedLevel, string adminName, string userName, int sourceId)
        {
            ...
            return await ImportContentsAsync(channel, siteContentDirectoryPath, isOverride, taxis, isChecked, checkedLevel, adminName, userName, sourceId);
        }
        ...
        public async Task<List<int>> ImportContentsByXlsxFileAsync(Channel channel, string filePath, List<string> attributes, bool isOverride, bool isChecked, int checkedLevel, string adminName, string userName, int sourceId)
        {
                ...
                for (var i = rowIndex; i < sheet.Rows.Count; i++) //行
                {
                    ...
                    if (!string.IsNullOrEmpty(content.Title))
                    {
                        ...
                        if (!string.IsNullOrEmpty(channel1Title) && !string.IsNullOrEmpty(channel2Title))
                        {
                            ...
                            if (channel1 == null)
                            {
                                var channel1Id = await _databaseManager.ChannelRepository.InsertAsync(_site.Id, channel.Id, channel1Title, string.Empty, channel.ContentModelPluginId, channel.ChannelTemplateId, channel.ContentTemplateId, channel.Knowledge);
                            ...
                            if (channel2 == null)
                            {
                                var channel2Id = await _databaseManager.ChannelRepository.InsertAsync(_site.Id, channel1.Id, channel2Title, string.Empty, channel1.ContentModelPluginId, channel1.ChannelTemplateId, channel1.ContentTemplateId, channel1.Knowledge);
        ...
        public async Task<List<int>> ImportContentsByImageFileAsync(Channel channel, string fileName, string fileUrl, bool isOverride, bool isChecked, int checkedLevel, string adminName, string userName, int sourceId)
        {
        ...
        public async Task<List<int>> ImportContentsByTxtFileAsync(Channel channel, string txtFilePath, bool isOverride, bool isChecked, int checkedLevel, string adminName, string userName, int sourceId)
        {
            var contentInfo = new Content
            {
                SiteId = channel.SiteId,
        ...
        private async Task<List<int>> ImportContentsAsync(Channel channel, string siteContentDirectoryPath, bool isOverride, int taxis, bool isChecked, int checkedLevel, string adminName, string userName, int sourceId)
        {
            ...
            var contentIdList = await contentIe.ImportContentsAsync(filePath, isOverride, channel, taxis, isChecked, checkedLevel, adminName, userName, sourceId);
