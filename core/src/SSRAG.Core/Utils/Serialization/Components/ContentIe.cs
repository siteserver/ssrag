...
using SSRAG.Core.Utils.Serialization.Atom.Atom.Core.Collections;
using SSRAG.Enums;
using SSRAG.Models;
...
namespace SSRAG.Core.Utils.Serialization.Components
{
    internal class ContentIe
    {
        ...
        public async Task<List<int>> ImportContentsAsync(string filePath, bool isOverride, Channel channel, int taxis, bool isChecked, int checkedLevel, string adminName, string userName, int sourceId)
        {
            if (!FileUtils.IsFileExists(filePath)) return null;
            var feed = AtomFeed.Load(FileUtils.GetFileStreamReadOnly(filePath));

            return await ImportContentsAsync(feed.Entries, channel, taxis, false, isChecked, checkedLevel, isOverride, adminName, userName, sourceId);
        }

        public async Task ImportContentsAsync(AtomEntryCollection entries, Channel channel, int taxis, bool isOverride, string adminName, string uuid)
        {
            await ImportContentsAsync(entries, channel, taxis, 0, 0, true, true, 0, isOverride, adminName, uuid);
        }

        private async Task ImportContentsAsync(AtomEntryCollection entries, Channel channel, int taxis, int importStart, int importCount, bool isCheckedBySettings, bool isChecked, int checkedLevel, bool isOverride, string adminName, string uuid)
        {
            ...
            foreach (AtomEntry entry in entries)
            {
                try
                {
                    ...
                    {
                        templateId = await _databaseManager.TemplateRepository.GetTemplateIdByTemplateNameAsync(_site.Id, TemplateType.ContentTemplate, templateName);
                    }
            ...
            {
                await _caching.SetProcessAsync(uuid, $"导入内容: {content.Title}");
                try
                {
                    if (content.Id > 0)
                    {
        ...
        private async Task<List<int>> ImportContentsAsync(AtomEntryCollection entries, Channel channel, int taxis, bool isCheckedBySettings, bool isChecked, int checkedLevel, bool isOverride, string adminName, string userName, int sourceId)
        {
            var contents = new List<Content>();
            ...
            foreach (AtomEntry entry in entries)
            {
                try
                {
                    ...
                    var linkType = TranslateUtils.ToEnum<LinkType>(AtomUtility.GetDcElementContent(entry.AdditionalElements, nameof(Content.LinkType)), LinkType.None);
                    var linkUrl = AtomUtility.Decrypt(AtomUtility.GetDcElementContent(entry.AdditionalElements, nameof(Content.LinkUrl)));
                    // var linkToChannelName = string.Empty;
                    // if (linkType == LinkType.LinkToChannel)
                    // {
                    //     linkToChannelName = AtomUtility.GetDcElementContent(entry.AdditionalElements, LinkToChannelName); 
                    // }
        ...
        public async Task<bool> ExportContentsAsync(Site site, List<Content> contents, List<TableStyle> tableStyles)
        {
            ...
            foreach (var content in contents)
            {
                try
                {
                    _pathManager.PutFilePaths(site, content, collection, tableStyles);
                }
                catch
                {
                    // ignored
                }
                var entry = await ExportContentAsync(content);
        ...
        public async Task<AtomEntry> ExportContentAsync(Content content)
        {
            ...
            AtomUtility.AddDcElement(entry.AdditionalElements, nameof(Content.LinkType), content.LinkType.GetValue());
            ...
            {
                var templateName = await _databaseManager.TemplateRepository.GetTemplateNameAsync(content.TemplateId);
                AtomUtility.AddDcElement(entry.AdditionalElements, TemplateName, templateName);
            }
