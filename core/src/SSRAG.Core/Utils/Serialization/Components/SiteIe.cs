...
namespace SSRAG.Core.Utils.Serialization.Components
{
    internal class SiteIe
    {
        ...
        public async Task<int> ImportChannelsAndContentsAsync(string filePath, bool isImportContents, bool isOverride, int theParentId, string adminName, string uuid)
        {
            ...
            if (parentIdOriginal == 0)
            {
                ...
                await _caching.SetProcessAsync(uuid, $"导入栏目：{channel.ChannelName}");
                await _databaseManager.ChannelRepository.UpdateAsync(channel);

                if (isImportContents)
                {
                    await _contentIe.ImportContentsAsync(feed.Entries, channel, 0, isOverride, adminName, uuid);
                }
            }
            else
            {
