...
using SSRAG.Core.Utils.Serialization.Atom.Atom.AdditionalElements;
using SSRAG.Core.Utils.Serialization.Atom.Atom.AdditionalElements.DublinCore;
...
namespace SSRAG.Core.Utils.Serialization.Components
{
    internal class FormIe
    {
        ...
        public FormIe(IPathManager pathManager, IDatabaseManager databaseManager, Caching caching, int siteId, string directoryPath)
        {
        ...
        public async Task ImportFormsAsync(string uuid)
        {
            ...
            {
                await _caching.SetProcessAsync(uuid, $"导入表单文件: {directoryPath}");
        ...
        private AtomFeed GetEmptyFeed()
        {
            var feed = new AtomFeed
            {
                ...
                Author = new AtomPersonConstruct("author",
                    "siteserver", new Uri(Constants.OfficialHost)),
        ...
        private object GetValue(ScopedElementCollection additionalElements, TableColumn tableColumn)
        {
            ...
            {
                return DesEncryptor.DecryptStringBySecretKey(GetDcElementContent(additionalElements, tableColumn.AttributeName), "TgQQk42O");
            }
        ...
        private void SetValue(ScopedElementCollection additionalElements, TableColumn tableColumn, Entity entity)
        {
            ...
            if (tableColumn.DataType == DataType.Text)
            {
                value = DesEncryptor.EncryptStringBySecretKey(value, "TgQQk42O");
            }
