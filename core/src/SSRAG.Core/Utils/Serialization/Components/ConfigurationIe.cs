...
namespace SSRAG.Core.Utils.Serialization.Components
{
    public class ConfigurationIe
    {
        ...
        public async Task ExportAsync()
        {
            ...
            AtomUtility.AddDcElement(feed.AdditionalElements, nameof(Site.SiteType), _site.SiteType.GetValue());
        ...
        public async Task ImportAsync(string uuid)
        {
            ...
            await _caching.SetProcessAsync(uuid, "更新知识库配置...");
            ...
            var channelGroupIe = new ChannelGroupIe(_databaseManager, _caching);
            await channelGroupIe.ImportAsync(feed, _site.Id, uuid);

            var contentGroupIe = new ContentGroupIe(_databaseManager, _caching);
            await contentGroupIe.ImportAsync(feed, _site.Id, uuid);

            var contentTagIe = new ContentTagIe(_databaseManager, _caching);
            await contentTagIe.ImportAsync(feed, _site.Id, uuid);
        }
