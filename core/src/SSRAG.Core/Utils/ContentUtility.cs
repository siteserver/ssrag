...
namespace SSRAG.Core.Utils
{
    public static class ContentUtility
    {
        ...
        public static async Task TranslateAsync(IPathManager pathManager, IDatabaseManager databaseManager, IPluginManager pluginManager, Site site, int channelId, int contentId, int targetSiteId, int targetChannelId, TranslateType translateType, ICreateManager createManager, string adminName)
        {
            ...
            else if (translateType == TranslateType.Cut)
            {
                ...
                await databaseManager.ContentRepository.TrashContentAsync(site, channel, contentId, adminName);
