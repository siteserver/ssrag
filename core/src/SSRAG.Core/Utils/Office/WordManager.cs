...
using SSRAG.Core.Utils.Office.Word2Html;
using SSRAG.Enums;
using SSRAG.Models;
using SSRAG.Services;
using SSRAG.Utils;

namespace SSRAG.Core.Utils.Office
{
    public class WordManager
    {
        ...
        public void Init(IPathManager pathManager, Site siteInfo)
        {
            if (siteInfo != null)
            {
                ImageDirectoryPath = pathManager.GetUploadDirectoryPath(siteInfo, UploadType.Image);
                ImageDirectoryUrl = pathManager.GetSiteUrlByPhysicalPath(siteInfo, ImageDirectoryPath, true);
            }
            else
            {
        ...
        public async Task ParseAsync(IPathManager pathManager, Site siteInfo)
        {
            ...
            SSRAG.Utils.FileUtils.DeleteFileIfExists(DocsFilePath);

            if (Site != null)
            {
                Body = pathManager.DecodeTextEditor(Site, Body, true);
            }
        ...
        private string UploadImageUrlDelegate(byte[] imgByte, string picType)
        {
            ...
            var imageFileName = StringUtils.GetShortUuid(false) + "." + extension;
