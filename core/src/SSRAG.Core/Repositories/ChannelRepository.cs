...
namespace SSRAG.Core.Repositories
{
    public partial class ChannelRepository : IChannelRepository
    {
        ...
        {
            _repository = new Repository<Channel>(settingsManager.Database, settingsManager.Cache);
        ...
        public async Task<int> InsertAsync(int siteId, int parentId, string channelName, string indexName, string contentModelPluginId, int channelTemplateId, int contentTemplateId, bool isKnowledge)
        {
        ...
        public async Task DeleteAsync(Site site, int channelId)
        {
                ...
                if (groupArr.Length > 0)
                {
                    ...
                    foreach (var theGroup in groupArr)
                    {
                        ...
                        whereStringBuilder.Append(
                                $" ({TableName}.GroupNames = '{trimGroup}' OR {DbUtils.GetInStr(_settingsManager.DatabaseType, $"{TableName}.GroupNames", trimGroup + ",")} OR {DbUtils.GetInStr(_settingsManager.DatabaseType, $"{TableName}.GroupNames", "," + trimGroup + ",")} OR {DbUtils.GetInStr(_settingsManager.DatabaseType, $"{TableName}.GroupNames", "," + trimGroup)}) OR ");
                    }
                    if (groupArr.Length > 0)
                    {
            ...
            if (!string.IsNullOrEmpty(groupNot))
            {
                ...
                if (groupNotArr.Length > 0)
                {
                    ...
                    foreach (var theGroupNot in groupNotArr)
                    {
                        ...
                        whereStringBuilder.Append(
                                $" ({TableName}.GroupNames <> '{trimGroupNot}' AND {DbUtils.GetNotInStr(_settingsManager.DatabaseType, $"{TableName}.GroupNames", trimGroupNot + ",")} AND {DbUtils.GetNotInStr(_settingsManager.DatabaseType, $"{TableName}.GroupNames", "," + trimGroupNot + ",")} AND {DbUtils.GetNotInStr(_settingsManager.DatabaseType, $"{TableName}.GroupNames", "," + trimGroupNot)}) AND ");
                    }
                    if (groupNotArr.Length > 0)
                    {
