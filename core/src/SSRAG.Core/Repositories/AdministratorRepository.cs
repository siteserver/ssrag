...
namespace SSRAG.Core.Repositories
{
    public partial class AdministratorRepository : IAdministratorRepository
    {
            ...
            IAdministratorsInRolesRepository administratorsInRolesRepository, IRoleRepository roleRepository)
        {
            ...
            _repository = new Repository<Administrator>(settingsManager.Database, settingsManager.Cache);
        ...
        public async Task<List<string>> GetAdministratorUserNamesAsync(string keyword)
        {
            var query = Q.Select(nameof(Administrator.UserName));
            if (!string.IsNullOrEmpty(keyword))
            {
                var like = $"%{keyword}%";
                query.Where(q => q
