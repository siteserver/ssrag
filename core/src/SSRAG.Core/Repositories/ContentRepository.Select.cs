...
namespace SSRAG.Core.Repositories
{
    public partial class ContentRepository
    {
        ...
        {
            return _settingsManager.Cache.GetEntityKey(tableName, contentId);
        }

        private string GetListKey(string tableName, int siteId, int channelId)
        {
            return _settingsManager.Cache.GetListKey(tableName, siteId.ToString(), (Math.Abs(channelId)).ToString());
        }
        ...
        public async Task ClearAllListCacheAsync(Site site)
        {
            ...
            foreach (var channel in channelSummaries)
            {
                ...
                await _settingsManager.Cache.RemoveAsync(listKey);
            }
        }
        ...
        public async Task CacheAllListAndCountAsync(Site site, List<ChannelSummary> channelSummaries)
        {
            ...
            foreach (var channel in channelSummaries)
            {
                ...
                if (!await _settingsManager.Cache.ExistsAsync(listKey))
                {
                    var query = GetSummariesQuery(site, channel);
                    var summaries = await repository.GetAllAsync<ContentSummary>(query);

                    await _settingsManager.Cache.SetObjectAsync(listKey, summaries);
                }
            }
        }
        ...
        public async Task CacheAllEntityAsync(Site site, List<ChannelSummary> channelSummaries)
        {
            ...
            foreach (var channel in channelSummaries)
            {
                ...
                if (summary != null)
                {
                    ...
                    if (!await _settingsManager.Cache.ExistsAsync(GetEntityKey(tableName, summary.Id)))
                    {
                        ...
                        {
                            await _settingsManager.Cache.SetObjectAsync(GetEntityKey(tableName, content.Id), content);
                        }
                    }
                }
            }
