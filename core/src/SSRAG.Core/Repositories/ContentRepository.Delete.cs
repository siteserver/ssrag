...
namespace SSRAG.Core.Repositories
{
    ...
    {
        public async Task TrashContentsAsync(Site site, Channel channel, List<int> contentIdList, string adminName)
        {
            ...
            await repository.UpdateAsync(
                GetQuery(site.Id, channel.Id)
                    ...
                    .Set(nameof(Content.LastEditAdminName), adminName)
                    .WhereIn(nameof(Content.Id), contentIdList)
                    .CachingRemove(cacheKeys.ToArray())
            );
        }

        public async Task TrashContentsAsync(Site site, int channelId, string adminName)
        {
            ...
            foreach (var selectedId in channelIds)
            {
                ...
                await TrashContentsAsync(site, selected, contentIds, adminName);
            }
        }

        public async Task TrashContentAsync(Site site, Channel channel, int contentId, string adminName)
        {
            var repository = await GetRepositoryAsync(site, channel);
