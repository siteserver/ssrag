...
namespace SSRAG.Core.Services
{
    public partial class FormManager : IFormManager
    {
        ...
        public FormManager(IPathManager pathManager, IDatabaseManager databaseManager, ISettingsManager settingsManager, ISmsManager smsManager, IMailManager mailManager, IChannelRepository channelRepository, IContentRepository contentRepository, ITableStyleRepository tableStyleRepository, IFormRepository formRepository, IFormDataRepository formDataRepository)
        {
        ...
        public async Task SendNotifyAsync(Form form, List<TableStyle> styles, FormData data)
        {
                ...
                !string.IsNullOrEmpty(form.AdministratorMailNotifyAddress))
            {
                ...
                if (isMailEnabled)
                {
                    ...
                    {
                        new KeyValuePair<string, string>("编号", data.Uuid)
                    };
                    if (data.CreatedDate.HasValue)
                    {
                    ...
                    var subject = !string.IsNullOrEmpty(form.AdministratorMailTitle) ? form.AdministratorMailTitle : "[SSRAG] 通知邮件";
