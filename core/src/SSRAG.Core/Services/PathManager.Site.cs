...
namespace SSRAG.Core.Services
{
    public partial class PathManager
    {
        ...
        public string ParseSiteUrl(Site site, string virtualUrl, bool isLocal)
        {
            if (string.IsNullOrEmpty(virtualUrl)) return string.Empty;

            if (site != null && StringUtils.StartsWith(virtualUrl, "@"))
            {
                return GetSiteUrl(site, virtualUrl.Substring(1), isLocal);
            }

            return ParseUrl(virtualUrl);
        }

        public string ParseSitePath(Site site, string virtualPath)
        {
            ...
            if (site != null)
            {
                return GetSitePath(site, virtualPath.Substring(1));
            }
            return ParsePath(resolvedPath);
        }

        public string GetSiteUrl(Site site, bool isLocal)
        {
            return GetSiteUrl(site, string.Empty, isLocal);
        }

        public string GetSiteUrl(Site site, string requestPath, bool isLocal)
        {
            var url = isLocal
        ...
        public string GetSiteUrlByPhysicalPath(Site site, string physicalPath, bool isLocal)
        {
            if (site == null || string.IsNullOrEmpty(physicalPath)) return GetWebUrl(site);
        ...
        public string GetVirtualUrlByPhysicalPath(Site site, string physicalPath)
        {
            if (site == null || string.IsNullOrEmpty(physicalPath)) return GetWebUrl(site);
        ...
        public string GetRemoteSiteUrl(Site site, string requestPath)
        {
            var url = GetWebUrl(site);
            ...
            var assetsUrl = PageUtils.Combine(GetWebUrl(site),
                site.AssetsDir);
            if (StringUtils.StartsWithIgnoreCase(url, assetsUrl))
            {
                url = StringUtils.ReplaceStartsWithIgnoreCase(url, assetsUrl, GetAssetsUrl(site));
            }

            return url;
        }

        public string GetLocalSiteUrl(Site site, string requestPath = null)
        {
            var url = ParseUrl($"~/{(site.Root ? string.Empty : site.SiteDir)}");
        ...
        public async Task<string> GetIndexPageUrlAsync(Site site, bool isLocal)
        {
            ...
            var url = isLocal
                ...
                : ParseSiteUrl(site, createdFileFullName, false);
        ...
        private async Task<string> ParseContentUrlAsync(Site site, Content content)
        {
            ...
            {
                return ParseSiteUrl(site, content.LinkUrl, false);
            }
        ...
        public async Task<string> GetChannelUrlNotComputedAsync(Site site, int channelId, bool isLocal)
        {
            ...
            if (string.IsNullOrEmpty(linkUrl))
            {
                if (channel != null)
                {
                    ...
                    return ParseSiteUrl(site, AddVirtualToPath(filePath), isLocal);
                }
            }

            return ParseSiteUrl(site, linkUrl, isLocal);
        }
        ...
        public async Task<string> GetInputChannelUrlAsync(Site site, Channel node, bool isLocal)
        {
            ...
            channelUrl = StringUtils.ReplaceStartsWith(channelUrl, GetWebUrl(site), string.Empty);
        ...
        public string GetSitePath(Site site)
        {
            return GetSitePath(site, null);
        }

        public string GetSitePath(Site site, params string[] paths)
        {
            if (site == null)
            {
            ...
            var sitePath = PathUtils.Combine(_settingsManager.WebRootPath, site.Root ? string.Empty : site.SiteDir);
        ...
        public string GetIndexPageFilePath(Site site, string createFileFullName, bool root)
        {
            ...
            return ParseSitePath(site, createFileFullName);
        }
        ...
        public List<FileInfo> GetAllFilesOrderByCreationTimeDesc(Site site, UploadType uploadType)
        {
            ...
            var directoryPath = PathUtils.Combine(GetSitePath(site), uploadDirectoryName);
        ...
        public string GetUploadDirectoryPath(Site site, string fileExtension)
        {
            return GetUploadDirectoryPath(site, DateTime.Now, fileExtension);
        }

        public string GetUploadDirectoryPath(Site site, DateTime datetime, string fileExtension)
        {
        ...
        public string GetUploadDirectoryPath(Site site, UploadType uploadType)
        {
            return GetUploadDirectoryPath(site, DateTime.Now, uploadType);
        }

        public string GetUploadDirectoryPath(Site site, DateTime datetime, UploadType uploadType)
        {
        ...
        public async Task<string> SaveImageAsync(Site site, string content, string excludePrefix = null)
        {
            ...
            foreach (var originalImageSrc in originalImageSrcs)
            {
                if (!PageUtils.IsProtocolUrl(originalImageSrc) ||
                    ...
                    StringUtils.StartsWithIgnoreCase(originalImageSrc, GetWebUrl(site)))
                {
                    continue;
                }
                ...
                {
                    fileName = StringUtils.GetShortUuid(false) + fileExtName;
                }
                ...
                try
                {
                    ...
                    var fileUrl = GetSiteUrlByPhysicalPath(site, filePath, true);
        ...
        public string GetSiteTemplatesPath(SiteType siteType, string relatedPath)
        {
            relatedPath = PathUtils.RemoveParentPath(relatedPath);
            return PathUtils.Combine(_settingsManager.WebRootPath, DirectoryUtils.SiteFiles.DirectoryName, DirectoryUtils.SiteFiles.Templates.DirectoryName, siteType.GetValue(), relatedPath);
        }

        public string GetSiteTemplateMetadataPath(string siteTemplatePath, string relatedPath)
        {
            relatedPath = PathUtils.RemoveParentPath(relatedPath);
            return PathUtils.Combine(siteTemplatePath, DirectoryUtils.SiteFiles.Templates.SiteTemplateMetadata, relatedPath);
        }
        ...
        public async Task<string> GetChannelPageFilePathAsync(Site site, int channelId)
        {
            ...
            if (nodeInfo.ParentId == 0)
            {
                ...
                return GetIndexPageFilePath(site, templateInfo.CreatedFileFullName, site.Root);
            }
            ...
            filePath = ParseSitePath(site, filePath);// PathUtils.Combine(sitePath, filePath);
        ...
        public async Task<string> GetContentPageFilePathAsync(Site site, int channelId, Content content, int currentPageIndex)
        {
            ...
            filePath = ParseSitePath(site, filePath);
            if (PathUtils.IsDirectoryPath(filePath))
            {
        ...
        public async Task DeleteSiteFilesAsync(Site site)
        {
            ...
            if (site.Root)
            {
                ...
                var siteDirList = await _databaseManager.SiteRepository.GetSiteDirsAsync();
        ...
        public async Task<(bool success, string errorMessage)> ChangeParentSiteAsync(int oldParentSiteId, int newParentSiteId, int siteId, string siteDir)
        {
            ...
            if (oldParentSiteId != 0)
            {
                ...
                oldPsPath = GetSitePath(oldSite, siteDir);
            }
            else
            {
                var site = await _databaseManager.SiteRepository.GetAsync(siteId);
                oldPsPath = GetSitePath(site);
            }
            ...
            if (newParentSiteId != 0)
            {
                ...
                newPsPath = PathUtils.Combine(GetSitePath(newSite), siteDir);
            }
            else
            {
        ...
        public void AddWaterMark(Site site, string imagePath)
        {
            try
            {
                ...
                if (FileUtils.IsImage(fileExtName))
                {
                    if (site.IsWaterMark)
                    {
                        if (site.IsImageWaterMark)
                        {
                            ...
                            {
                                ImageUtils.AddImageWaterMark(imagePath, ParseSitePath(site, site.WaterMarkImagePath), site.WaterMarkPosition, site.WaterMarkTransparency, site.WaterMarkMinWidth, site.WaterMarkMinHeight);
                            }
                        }
                        else
                        {
        ...
        public void MoveFile(Site sourceSite, Site destSite, string relatedUrl)
        {
            if (!string.IsNullOrEmpty(relatedUrl))
            {
                var sourceFilePath = ParseSitePath(sourceSite, relatedUrl);
                var descFilePath = ParseSitePath(destSite, relatedUrl);
        ...
        public void MoveFileByChannel(Site sourceSite, Site destSite, Channel channel)
        {
