...
using Dapper;
using SSRAG.Datory.DatabaseImpl;
...
namespace SSRAG.Datory
{
    public class Database : IDatabase
    {
        ...
        public async Task CreateTableAsync(string tableName, IEnumerable<TableColumn> tableColumns)
        {
            ...
            var quotedTableName = GetQuotedIdentifier(Utilities.FilterSql(tableName));

            sqlBuilder.Append($@"CREATE TABLE {quotedTableName} (").AppendLine();

            var primaryKeyColumns = new List<TableColumn>();
            TableColumn identityColumn = null;
            var isUuid = true;
            if (Utilities.EqualsIgnoreCase(tableName, "ssrag_segment"))
            {
                isUuid = false;
            }
            if (!isUuid)
            {
                tableColumns = tableColumns.Where(tableColumn => !Utilities.EqualsIgnoreCase(tableColumn.AttributeName, nameof(Entity.Uuid)));
            }
            ...
            foreach (var tableColumn in tableColumns)
            {
                ...
                else if (Utilities.EqualsIgnoreCase(tableColumn.AttributeName, nameof(Entity.Uuid)))
                {
                    tableColumn.DataType = DataType.VarChar;
                    tableColumn.DataLength = 50;
                }
            ...
            if (DatabaseType != DatabaseType.SQLite && DatabaseType != DatabaseType.Kingbase)
            {
                ...
                {
                    var primaryKeySql = DbUtils.GetPrimaryKeySqlString(DatabaseType, quotedTableName, identityColumn.AttributeName);
                ...
                else if (primaryKeyColumns.Count > 0)
                {
                    ...
                    {
                        var primaryKeySql = DbUtils.GetPrimaryKeySqlString(DatabaseType, quotedTableName, tableColumn.AttributeName);
                        if (!string.IsNullOrEmpty(primaryKeySql))
                        {
                            sqlBuilder.Append(primaryKeySql).Append(",");
                        }
        ...
        public List<TableColumn> GetTableColumns(IEnumerable<TableColumn> tableColumns)
        {
            var columns = new List<TableColumn>
            {
                ...
                new TableColumn
                {
                    AttributeName = nameof(Entity.Uuid),
            ...
            if (tableColumns != null)
            {
                columns.AddRange(tableColumns.Where(tableColumn =>
                    ...
                    !Utilities.EqualsIgnoreCase(tableColumn.AttributeName, nameof(Entity.Uuid)) &&
