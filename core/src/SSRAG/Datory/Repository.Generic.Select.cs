...
namespace SSRAG.Datory
{
    public partial class Repository<T> where T : Entity, new()
    {
        ...
        public virtual async Task<T> GetAsync(string uuid, Query query = null)
        {
            query ??= Q.NewQuery();
            return !Utilities.IsUuid(uuid) ? null : await GetAsync(query.Where(nameof(Entity.Uuid), uuid));
        }

        public virtual async Task<T> GetAsync(Query query = null)
        {
            return await RepositoryUtils.GetObjectAsync<T>(Database, TableName, Cache, query);
        }

        public virtual async Task<List<T>> GetAllAsync(Query query = null)
        {
            return await RepositoryUtils.GetObjectListAsync<T>(Database, TableName, Cache, query);
        }

        public virtual async Task<TValue> GetAsync<TValue>(Query query)
        {
            return await RepositoryUtils.GetValueAsync<TValue>(Database, TableName, Cache, query);
        }

        public virtual async Task<List<TValue>> GetAllAsync<TValue>(Query query = null)
        {
            return await RepositoryUtils.GetValueListAsync<TValue>(Database, TableName, Cache, query);
        }
