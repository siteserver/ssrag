...
namespace SSRAG.Utils
{
    public static class StringUtils
    {
        ...
        /// </summary>
        /// <param name="inputBytes">The input bytes to convert</param>
        /// <returns>The Base64URL encoded string</returns>
        public static string BytesToBase64Url(byte[] inputBytes)
        {
            // Convert to base64
            string base64Str = Convert.ToBase64String(inputBytes);
            // Convert to Base64URL format
            return base64Str.Replace("+", "-").Replace("/", "_").TrimEnd('=');
        }

        /// <summary>
        /// Convert a Base64URL string back to bytes.
        /// </summary>
        ...
        public static byte[] Base64UrlToBytes(string base64Url)
        {
            ...
            string base64Str = base64Url.Replace("-", "+").Replace("_", "/");
            // Decode base64
            return Convert.FromBase64String(base64Str);
        }

        public static string GetShortUuid(bool isUppercase)
        {
            long i = 1;
            foreach (var b in Guid.NewGuid().ToByteArray())
            {
        ...
        public static string ReplaceNewlineToEmpty(string inputString)
        {
            if (string.IsNullOrEmpty(inputString)) return string.Empty;
            var retVal = new StringBuilder();
            inputString = inputString.Trim();
