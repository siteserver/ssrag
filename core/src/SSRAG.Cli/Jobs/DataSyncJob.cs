...
namespace SSRAG.Cli.Jobs
{
    public class DataSyncJob : IJobService
    {
        ...
        public DataSyncJob(ISettingsManager settingsManager, IDatabaseManager databaseManager)
        {
            ...
            {
                {
                    "from=", "Specify the file name of ssrag.json configuration file that you want to backup",
                    v => _from = v
                },
                {
                    "to=", "Specify the file name of ssrag.json configuration file that you want to restore",
                    v => _to = v
                },
                {
        ...
        public async Task WriteUsageAsync(IConsoleUtils console)
        {
            ...
            await console.WriteLineAsync($"Docs: {Constants.OfficialHost}/docs/cli/commands/data-sync.html");
        ...
        public async Task ExecuteAsync(IPluginJobContext context)
        {
            ...
            {
                await console.WriteErrorAsync($"The backup ssrag configuration file does not exist: {backupConfigPath}");
                return;
            }
            var restoreConfigPath = PathUtils.Combine(_settingsManager.ContentRootPath, _to);
            if (!FileUtils.IsFileExists(restoreConfigPath))
            {
                await console.WriteErrorAsync($"The restore ssrag configuration file does not exist: {restoreConfigPath}");
                return;
            }
